...
(function () {
  ...
  function search() {
    ...
    searchHits.forEach(function (page) {
      ...
      const a = li.querySelector('a'), small = element('<small></small>');

      a.href = page.href;
      a.textContent = page.title;
      const content = page.content;
      const index =  content.indexOf(input.value)
      const sub =  content.substring(index < 10 ? index : index - 10, index + 20);

      small.innerHTML = sub.replace(input.value, '<span style="color:red">'+ input.value + '</span>');
